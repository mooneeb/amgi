...
- **Marvin name-to-ID resolution** — When a Marvin config uses `list_name` or `label_names`, AMGI currently returns an error. The full behaviour is to call `GET /api/categories` and `GET /api/labels`, match by title, cache the results, and use the resolved IDs at task-creation time. Requires: a reads rate limiter (1 req / 3 sec per Marvin's published limits), a small cache with reload semantics, and graceful handling of the "name not found" case. Estimated 0.5-1 day of work. Work around for now by supplying `list_id` and `label_ids` directly (copy from Marvin's URL bar).
...
- **`GITHUB_WEBHOOK_SECRET` is required in all deployment modes.** The env var check is currently unconditional; polling-only deployments must still set it to any non-empty value. To be fixed so the requirement is conditional on webhook mode being configured.
