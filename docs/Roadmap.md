...
- **Multi-tenancy (per-org webhook paths)** — Today we use a single webhook endpoint for all orgs. A future option could allow different paths per organization (e.g. `/webhooks/acme`, `/webhooks/other-org`) for isolation or routing. Most production environments use a single path and route by payload; this would add complexity and is deferred.
