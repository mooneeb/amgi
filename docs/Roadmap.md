...
- **Reverse-proxy / TLS reference manifests** — webhook deployments need TLS termination and HTTP routing in front of the AMGI container. v1.0 ships a starter Kubernetes Ingress (requires the operator to supply the ingress class, hostname, and TLS secret); the Docker Compose path leaves reverse-proxy setup entirely to the operator. Advanced patterns — cert-manager integration for automatic TLS, a docker-compose override for Traefik/Caddy, multi-ingress-class examples — are post-1.0.
...
- **No shipped reverse-proxy / TLS reference manifests** — webhook deployments need TLS termination. Kubernetes ships a starter Ingress that operators customize with their own class, host, and TLS secret; Docker Compose has no reverse-proxy starter in v1.0. See [section 2.3](#23-operational-maturity).
